...
public class ActionClient<T_ACTION_FEEDBACK extends Message, T_ACTION_GOAL extends Message, T_ACTION_RESULT extends Message, T_FEEDBACK extends Message, T_GOAL extends Message, T_RESULT extends Message> {
  ...
  public void addClientPubSub(ConnectedNode node) {
    ...
    subFeedback =
        node.newSubscriber(this.nameSpace + "/" + ActionConstants.TOPIC_NAME_FEEDBACK, spec.getActionFeedbackMessage());
    ...
    subResult =
        node.newSubscriber(this.nameSpace + "/" + ActionConstants.TOPIC_NAME_RESULT, spec.getActionResultMessage());
    ...
    subStatus =
        node.newSubscriber(this.nameSpace + "/" + ActionConstants.TOPIC_NAME_STATUS, ActionConstants.MESSAGE_TYPE_STATUS);
    subStatus.addMessageListener(statusCallback);

    pubGoal = node.newPublisher(this.nameSpace + "/" + ActionConstants.TOPIC_NAME_GOAL, spec.getActionGoalMessage());
    pubGoal.addListener(goalPublisherListener);
    pubCancelGoal =
        node.newPublisher(this.nameSpace + "/" + ActionConstants.TOPIC_NAME_CANCEL, ActionConstants.MESSAGE_TYPE_CANCEL);
  ...
  public void cancelGoalsAtAndBeforeTime(Time time) {
    ...
    GoalID cancelMessage = pubCancelGoal.newMessage();
    cancelMessage.setId( "");
    cancelMessage.setStamp(time);
  ...
  protected void doStatusCallback(GoalStatusArray goalStatuses) {
    ...
    String currCallerID = goalStatuses.getHeader().getFrameId();
    ...
    latestStatusTime = goalStatuses.getHeader().getStamp();
