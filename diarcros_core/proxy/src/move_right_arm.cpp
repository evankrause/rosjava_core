...
namespace diarc {
  namespace r_arm {
    ...
    /**
     * Given a pose, send it to the action server for the right arm,
    ...
    void actionGoalCallback(const arm_navigation_msgs::SimplePoseConstraint::ConstPtr& desired_pose) {
      ...
      ROS_INFO("RIGHT ARM :: Goal Pose (%.4f, %.4f, %.4f) :: Planning attempts: %d",
    ...
    void noCollisionCheckCB(const arm_navigation_msgs::SimplePoseConstraint::ConstPtr& dp) {
      ...
      ik_req.ik_request.ik_seed_state.joint_state.position.resize(diarc::r_arm::response.kinematic_solver_info.joint_names.size());
      ik_req.ik_request.ik_seed_state.joint_state.name = diarc::r_arm::response.kinematic_solver_info.joint_names;
      
      // seed an initial guess for IK
      for(unsigned int i = 0; i < diarc::r_arm::response.kinematic_solver_info.joint_names.size(); i++) {
        ik_req.ik_request.ik_seed_state.joint_state.position[i] =
          (diarc::r_arm::response.kinematic_solver_info.limits[i].min_position +
           diarc::r_arm::response.kinematic_solver_info.limits[i].max_position) / 2.0;
      }


      // Construct a joint goal if IK succeeds      
      ROS_INFO("Request IK solution");
      if(diarc::r_arm::ik_client.call(ik_req, ik_res) &&
...
int main(int argc, char** argv) {
  //init the ROS node
  ros::init(argc, argv, "r_arm_proxy");
  ros::NodeHandle nh;
  
  // Initialize the client for the Action interface to the head controller
  diarc::r_arm::pose_client = new diarc::r_arm::MoveArmClient("/move_right_arm", true);
  diarc::r_arm::traj_client = new diarc::r_arm::TrajClient("/r_arm_controller/joint_trajectory_action", true);
  
  // wait for head controller action server to come up
  while (!diarc::r_arm::pose_client->waitForServer(ros::Duration(5.0)) &&
         !diarc::r_arm::traj_client->waitForServer(ros::Duration(5.0))) {
    ROS_INFO("Waiting for the MoveArm & Trajectory server to come up");
  }


  // Wait for IK services
  ros::service::waitForService("pr2_right_arm_kinematics_nocoll/get_ik_solver_info");
  ros::service::waitForService("pr2_right_arm_kinematics_nocoll/get_ik");

  diarc::r_arm::query_client =
    nh.serviceClient<kinematics_msgs::GetKinematicSolverInfo>("pr2_right_arm_kinematics_nocoll/get_ik_solver_info");
  diarc::r_arm::ik_client =
    nh.serviceClient<kinematics_msgs::GetPositionIK>("pr2_right_arm_kinematics_nocoll/get_ik");

  // Get IK solver info
  if(diarc::r_arm::query_client.call(diarc::r_arm::request, diarc::r_arm::response)) {
    for(unsigned int i = 0; i < diarc::r_arm::response.kinematic_solver_info.joint_names.size(); i++) {
      ROS_DEBUG("Joint: %d %s", i, diarc::r_arm::response.kinematic_solver_info.joint_names[i].c_str());
    }
  }
  else {
  ...
  // Result publishers
  diarc::r_arm::pose_result_pub = nh.advertise<actionlib_msgs::GoalStatus>("/DIARC/proxy/move_right_arm/result", 10);
  diarc::r_arm::traj_result_pub = nh.advertise<actionlib_msgs::GoalStatus>("/DIARC/proxy/r_arm_joint/result", 10);
  

  // Goal subscribers
  ros::Subscriber pose_sub = nh.subscribe("/DIARC/proxy/move_right_arm/goal", 10, diarc::r_arm::actionGoalCallback);
  ros::Subscriber traj_sub = nh.subscribe("/DIARC/proxy/move_right_arm_nocoll/goal", 10, diarc::r_arm::noCollisionCheckCB);
  ROS_INFO("Right arm proxy initialized");
  ros::spin();
