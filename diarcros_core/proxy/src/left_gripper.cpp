...
int main(int argc, char** argv) {
  ...
  diarc::left_gripper::gripper_client_ = new diarc::left_gripper::GripperClient("/l_gripper_controller/gripper_action", true);
  
  //wait for head controller action server to come up
  while (!diarc::left_gripper::gripper_client_->waitForServer(ros::Duration(5.0))) {
    ROS_INFO("Waiting for the move_arm_client server to come up");
  }
  
  //publish and subscribe
  diarc::left_gripper::action_state_pub = n.advertise<actionlib_msgs::GoalStatus>("/DIARC/proxy/left_gripper/result", 10);
  ros::Subscriber action_goal_sub = n.subscribe("/DIARC/proxy/left_gripper/goal", 10, diarc::left_gripper::actionGoalCallback);
  ros::spin();
}
