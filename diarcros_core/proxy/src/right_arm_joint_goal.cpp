...
{
  // Init the ROS node
  ros::init(argc, argv, "DIARC/proxy/r_arm_joint_controller");
  ros::NodeHandle nh;

  diarc::r_arm_traj::r_traj_client = new diarc::r_arm_traj::TrajClient("r_arm_controller/joint_trajectory_action", true);
  while (!diarc::r_arm_traj::r_traj_client->waitForServer(ros::Duration(5.0))) {
    ROS_INFO("Waiting for the r_arm_joint_controller proxy to come up");
  }

  // setup pub/sub
  diarc::r_arm_traj::result_pub = nh.advertise<actionlib_msgs::GoalStatus>("/DIARC/proxy/r_arm_joint/result", 10);
  ros::Subscriber sub = nh.subscribe("/DIARC/proxy/r_arm_joint/goal", 10, diarc::r_arm_traj::callback);
  ros::spin();
}
