package edu.tufts.hrilab.diarcros;
 ...
 */
public class MessageDiarcrosInterfaceBuilder extends MessageInterfaceBuilder {
  ...
  public String build(MessageFactory messageFactory) {
    ...
    if (getPackageName() != null) {
      builder.append(String.format("package edu.tufts.hrilab.diarcros.msg.%s;\n\n", getPackageName()));
    }
...
//            System.out.println(messageContext.getName());
  ...
  /**
   * Converts an DIARC message back to the ROS version. This is slightly more
  ...
  private void appendAdeToRosConverter(MessageContext messageContext, StringBuilder builder) {
    ...
    builder.append(String.format("  public static %s.%s toRos(edu.tufts.hrilab.diarcros.msg.%s.%s val, ConnectedNode node) {\n", getPackageName(), getInterfaceName(),
            getPackageName(), getInterfaceName()));
    ...
    for (Field field : fields.getFields()) {
      ...
      if (isPrimitiveType(determineType(field))) {
        ...
        if (!isPrimitiveType(containedType)) {
          containedType = "edu.tufts.hrilab.diarcros.msg." + containedType.replace("org.ros.message.", "");
        }
        ...
        if (typeName.contains("Time") || type.contains("Duration")) {
          typeName = typeName.replace("edu.tufts.hrilab.diarcros.msg.", "org.ros.message.");
        }
  ...
  /**
   * Constructs the method which convets an DIARC message to its ROS equivalent.
   *
  ...
  private void appendRosToAdeConverter(MessageContext messageContext, StringBuilder builder) {
    ...
    builder.append(String.format("  public static edu.tufts.hrilab.diarcros.msg.%s.%s toAde(%s.%s val) {\n", getPackageName(), getInterfaceName(), getPackageName(), getInterfaceName()));
    builder.append(String.format("    edu.tufts.hrilab.diarcros.msg.%s %s = new edu.tufts.hrilab.diarcros.msg.%s();\n", type, instance, type));
    MessageFields fields = new MessageFields(messageContext);
    for (Field field : fields.getFields()) {
      if (field.isConstant()) {
        continue;
  ...
  /**
   * Constructs the method which converts an DIARC message to its ROS equivalent
   * by filling in passed in DIARC message (instead of returning a new one).
   *
   * @param messageContext the parsed representation of a ROS message
   * @param builder the file being generated
   */
  private void appendRosToAdeCopyConverter(MessageContext messageContext, StringBuilder builder) {
    ...
    builder.append(String.format("  public static void toAde(%s.%s val, edu.tufts.hrilab.diarcros.msg.%s %s) {\n", getPackageName(), getInterfaceName(), type, instance));
    MessageFields fields = new MessageFields(messageContext);
    for (Field field : fields.getFields()) {
      if (field.isConstant()) {
        continue;
  ...
  private String determineType(Field field) {
    ...
    if (getInterfaceName().equals("header") && type.equals("java.lang.String")) {
      return "edu.tufts.hrilab.diarcros.msg.std_msgs.String";
    }

    // full type name
    if (!isPrimitiveType(field.getJavaTypeName()) && !isContainer(field)) {
      type = "edu.tufts.hrilab.diarcros.msg." + type;
    }
    ...
    if (f != -1 && b != -1) {
      ...
      if (!isPrimitiveType(sub)) {
        type = type.substring(0, f) + ""
                + ""
                + "edu.tufts.hrilab.diarcros.msg." + sub + ">";
      }
    } else {
      type = type.replace("<", "<edu.tufts.hrilab.diarcros.msg.");
    }
