(ns edu.tufts.hrilab.rosade.core
  ^{:doc "Code for generating ROS/ADE bridge classes."
    :author "Jeremiah Via <jeremiah.via@gmail.com>"}
  (:use [clostache.parser :only [render-resource]]
        [clojure.tools.cli :only [cli]]
        [edu.tufts.hrilab.rosade.ros :only [rosnode-info]])
...
(def help-msg  "USAGE: ./gradlew rosade -Dnodename=/node -Dpackage=package")
...
(defn -main
  ...
  (when (< (count arg) 2)
    (println help-msg)
    (System/exit 0))
  (println "Generating a ROSADE node to communicate with" (nth arg 0))
  (try
    (when-let [node (ros->ade (nth arg 0) (nth arg 1))]
      (trace "Node representation complete: " node)
      (def filepath (str "../../rosade/" (package-path (:javapackage node)) "/" (:javanodename node) ".java"))
      ...
      (println (str "\n  File edu/tufts/hrilab/rosade/" (package-path (:javapackage node)) "/" (:javanodename node) ".java generated!")))
