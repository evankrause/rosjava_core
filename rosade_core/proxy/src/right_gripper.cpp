...
int main(int argc, char** argv) {
  //init the ROS node
  ros::init(argc, argv, "/ADE/proxy/right_gripper");
  
  ros::NodeHandle n;
  
  //Initialize the client for the Action interface to the head controller
  ade::right_gripper::gripper_client_ = new ade::right_gripper::GripperClient("/r_gripper_controller/gripper_action", true);
  
  //wait for head controller action server to come up
  while (!ade::right_gripper::gripper_client_->waitForServer(ros::Duration(5.0))) {
    ROS_INFO("Waiting for the move_arm_client server to come up");
  }
  
  //publish and subscribe
  ade::right_gripper::action_state_pub = n.advertise<actionlib_msgs::GoalStatus>("/ADE/proxy/right_gripper/result", 10);
  ros::Subscriber action_goal_sub = n.subscribe("/ADE/proxy/right_gripper/goal", 1, ade::right_gripper::actionGoalCallback);
  ros::spin();
}
