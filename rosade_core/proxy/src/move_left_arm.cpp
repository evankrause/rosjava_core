...
namespace ade {
  namespace l_arm {
    ...
    void noCollisionCheckCB(const arm_navigation_msgs::SimplePoseConstraint::ConstPtr& dp) {
      ...
      ik_req.ik_request.ik_seed_state.joint_state.position.resize(ade::l_arm::response.kinematic_solver_info.joint_names.size());
      ik_req.ik_request.ik_seed_state.joint_state.name = ade::l_arm::response.kinematic_solver_info.joint_names;
      
      // seed an initial guess for IK
      for(unsigned int i = 0; i < ade::l_arm::response.kinematic_solver_info.joint_names.size(); i++) {
        ik_req.ik_request.ik_seed_state.joint_state.position[i] =
          (ade::l_arm::response.kinematic_solver_info.limits[i].min_position +
           ade::l_arm::response.kinematic_solver_info.limits[i].max_position) / 2.0;
      }


      // Construct a joint goal if IK succeeds      
      ROS_INFO("Request IK solution");
      if(ade::l_arm::ik_client.call(ik_req, ik_res) &&
...
int main(int argc, char** argv) {
  ...
  ade::l_arm::pose_client = new ade::l_arm::MoveArmClient("/move_left_arm", true);
  ade::l_arm::traj_client = new ade::l_arm::TrajClient("/l_arm_controller/joint_trajectory_action", true);
  
  // wait for head controller action server to come up
  while (!ade::l_arm::pose_client->waitForServer(ros::Duration(5.0)) &&
         !ade::l_arm::traj_client->waitForServer(ros::Duration(5.0))) {
  ...
  // Get IK solver info
  if(ade::l_arm::query_client.call(ade::l_arm::request, ade::l_arm::response)) {
    for(unsigned int i = 0; i < ade::l_arm::response.kinematic_solver_info.joint_names.size(); i++) {
      ROS_DEBUG("Joint: %d %s", i, ade::l_arm::response.kinematic_solver_info.joint_names[i].c_str());
    }
  }
  else {
  ...
  // Result publishers
  ade::l_arm::pose_result_pub = nh.advertise<actionlib_msgs::GoalStatus>("/ADE/proxy/move_left_arm/result", 10);
  ade::l_arm::traj_result_pub = nh.advertise<actionlib_msgs::GoalStatus>("/ADE/proxy/l_arm_joint/result", 10);
  

  // Goal subscribers
  ros::Subscriber pose_sub = nh.subscribe("/ADE/proxy/move_left_arm/goal", 10, ade::l_arm::actionGoalCallback);
  ros::Subscriber traj_sub = nh.subscribe("/ADE/proxy/move_left_arm_nocoll/goal", 10, ade::l_arm::noCollisionCheckCB);
