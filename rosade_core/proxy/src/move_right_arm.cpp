...
namespace ade {
  namespace r_arm {
    ...
    void noCollisionCheckCB(const arm_navigation_msgs::SimplePoseConstraint::ConstPtr& dp) {
      ...
      ik_req.ik_request.ik_seed_state.joint_state.position.resize(ade::r_arm::response.kinematic_solver_info.joint_names.size());
      ik_req.ik_request.ik_seed_state.joint_state.name = ade::r_arm::response.kinematic_solver_info.joint_names;
      
      // seed an initial guess for IK
      for(unsigned int i = 0; i < ade::r_arm::response.kinematic_solver_info.joint_names.size(); i++) {
        ik_req.ik_request.ik_seed_state.joint_state.position[i] =
          (ade::r_arm::response.kinematic_solver_info.limits[i].min_position +
           ade::r_arm::response.kinematic_solver_info.limits[i].max_position) / 2.0;
      }


      // Construct a joint goal if IK succeeds      
      ROS_INFO("Request IK solution");
      if(ade::r_arm::ik_client.call(ik_req, ik_res) &&
...
int main(int argc, char** argv) {
  ...
  ade::r_arm::pose_client = new ade::r_arm::MoveArmClient("/move_right_arm", true);
  ade::r_arm::traj_client = new ade::r_arm::TrajClient("/r_arm_controller/joint_trajectory_action", true);
  
  // wait for head controller action server to come up
  while (!ade::r_arm::pose_client->waitForServer(ros::Duration(5.0)) &&
         !ade::r_arm::traj_client->waitForServer(ros::Duration(5.0))) {
  ...
  // Get IK solver info
  if(ade::r_arm::query_client.call(ade::r_arm::request, ade::r_arm::response)) {
    for(unsigned int i = 0; i < ade::r_arm::response.kinematic_solver_info.joint_names.size(); i++) {
      ROS_DEBUG("Joint: %d %s", i, ade::r_arm::response.kinematic_solver_info.joint_names[i].c_str());
    }
  }
  else {
  ...
  // Result publishers
  ade::r_arm::pose_result_pub = nh.advertise<actionlib_msgs::GoalStatus>("/ADE/proxy/move_right_arm/result", 10);
  ade::r_arm::traj_result_pub = nh.advertise<actionlib_msgs::GoalStatus>("/ADE/proxy/r_arm_joint/result", 10);
  

  // Goal subscribers
  ros::Subscriber pose_sub = nh.subscribe("/ADE/proxy/move_right_arm/goal", 10, ade::r_arm::actionGoalCallback);
  ros::Subscriber traj_sub = nh.subscribe("/ADE/proxy/move_right_arm_nocoll/goal", 10, ade::r_arm::noCollisionCheckCB);
