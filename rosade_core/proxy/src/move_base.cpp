...
ade::move_base::move_base_client = new ade::move_base::MoveBaseClient("move_base", true);
  while(!ade::move_base::move_base_client->waitForServer(ros::Duration(5.0))){
    ROS_INFO("Waiting for the move_base action server to come up");
  }

  // Set up publisher and subscriber
  ade::move_base::action_state_pub = node_handle.advertise<actionlib_msgs::GoalStatus>("/ADE/proxy/move_base/result", 10);
  ade::move_base::action_goal_sub  = node_handle.subscribe("/ADE/proxy/move_base/goal", 10, ade::move_base::mbCallback);
